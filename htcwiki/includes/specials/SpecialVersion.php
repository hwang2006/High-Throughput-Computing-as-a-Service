...
class SpecialVersion extends SpecialPage {
	...
	public static function getCopyrightAndAuthorList() {
		...
		$translatorsLink = '[//translatewiki.net/wiki/Translating:MediaWiki/Credits ' .
			wfMessage( 'version-poweredby-translators' )->text() . '((noopener noreferrer))]';
	...
	static function softwareInformation() {
		...
		$software = array();
		$software['[https://www.mediawiki.org/ MediaWiki((noopener noreferrer))]'] = self::getVersionLinked();
		$software['[http://www.php.net/ PHP((noopener noreferrer))]'] = phpversion() . " (" . PHP_SAPI . ")";
	...
	public function getEntryPointInfo() {
		...
		foreach ( $entryPoints as $message => $value ) {
			...
			$out .= Html::openElement( 'tr' ) .
				...
				Html::rawElement( 'td', array(), Html::rawElement( 'code', array(), "[$url $value((noopener noreferre))]" ) ) .
				Html::closeElement( 'tr' );
		}
